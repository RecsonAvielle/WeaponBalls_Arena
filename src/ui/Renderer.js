...
export class Renderer {
  ...
  render(balls, deathPops, spikes, projectiles, obstacles, trails, healBoxes, shockRings, bhData, minions, frostAreas, laserBeams, turrets, alpha, arena, extraData = null) {
    const { ctx } = this;
    ...
    for (const tur of turrets) this._drawTurret(tur);
    for (const ball of balls) {
      if (ball.alive && !ball.isBlight) this._drawBall(ball, alpha);
    }
  ...
  _drawTurret(tur) {
    ...
    let fadeOutAlpha = 1;
    let setupAlpha = 1;
    if (tur.state === 'SETUP') setupAlpha = 1 - Math.max(0, tur.timer / 2.5);
    else if (tur.state === 'LINGER') fadeOutAlpha = Math.min(1, tur.timer / 1.0);

    // Base is fully solid during setup, only fades out at the very end.
    ctx.globalAlpha = fadeOutAlpha;

    if (tur.state !== 'LINGER') {
      ctx.save();
      // Barrel fades in during setup, and fades out normally at the end.
      ctx.globalAlpha = fadeOutAlpha * setupAlpha;
      ctx.rotate(tur.aimAngle);
      ctx.fillStyle = tur.color;
      ctx.strokeStyle = '#2C2C2C';
      ctx.lineWidth = 1.5;
      const nw = tur.radius * 0.5;
      const nh = Math.max(4, tur.radius * 0.4);
      ctx.fillRect(0, -nh / 2, tur.radius + nw, nh);
      ctx.strokeRect(0, -nh / 2, tur.radius + nw, nh);
      ctx.restore();
    }
    ...
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.arc(0, 0, tur.radius, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();

    if (tur.state === 'SHOOTING') {
      ctx.strokeStyle = 'rgba(255, 255, 255, 0.4)';
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(0, 0, tur.radius - 3, 0, Math.PI * 2);
      ctx.stroke();
    }

    if (tur.state === 'SHOOTING' && tur.bulletCount > 0) {
      ctx.fillStyle = '#1A1A1A';
      ctx.font = `bold ${Math.max(12, tur.radius * 0.82)}px 'Outfit', sans-serif`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(String(tur.bulletCount), 0, 0);
    }
  ...
  _drawBall(ball, alpha) {
    ...
    if (shield > 0) {
      ...
      ctx.fillStyle = '#1A1A1A';
      ctx.font = `bold ${Math.max(12, r * 0.52)}px 'Outfit', sans-serif`;
      ctx.textAlign = 'center';
